...
for tournament in tournaments:
    cur.execute("INSERT INTO tournaments (name) VALUES (%s)", (tournament,))
...
for trainer_id in trainer_ids:
    ...
    for role_id in roles_assigned:
        try:
            cur.execute("""
                INSERT IGNORE INTO trainer_role (id_trainer, id_role) VALUES (%s, %s)
            """, (trainer_id, role_id))
        except:
            continue
conn.commit()
...
for _ in range(NUM_TRAINERS // 2):
    ...
    cur.execute("""
        ...
        VALUES (%s, %s, %s, %s)
    """, (trainer_id, item_type, json.dumps(details), random.choice([1, 1, 0])))
conn.commit()
...
for i in range(NUM_POKEMON):
    ...
    pokemon_ids.append(cur.lastrowid)
conn.commit()
...
for i in range(NUM_BATTLES):
    ...
    for move_name in moves_used:
        ...
        """, (
            ...
            1 if status in ['Scheduled', 'In Progress'] else random.choice([1, 0])
        ))
        move_ids.append(cur.lastrowid)
