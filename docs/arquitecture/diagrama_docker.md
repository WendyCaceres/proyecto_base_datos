...
flowchart TD
    ...
    subgraph ExternalAccess["External Access"]
        AppMaster["App / Client<br/>Port: 5432 → Master"]
        AppSlave1["Read Replica<br/>Port: 5437 → Slave1"]
        AppShard1["Shard Query<br/>Port: 5434 → Shard1"]
        AppShard2["Shard Query<br/>Port: 5435 → Shard2"]
        AppMySQL["App / Client<br/>Port: 3307 → MySQL"]
        AppMongo["App / Client<br/>Port: 27017 → MongoDB"]
        AppRedis["App / Client<br/>Port: 6379 → Redis"]
    end
